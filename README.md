...
- [Template architecture](#template-architecture)
  - [React 10](#react-10)
    - [React services & providers](#react-services--providers)
    - [React components](#react-components)
    - [React Universal (Server-Side-Rendering)](#react-universal-server-side-rendering)
  - [NodeJS](#nodejs)
...
- [Running on production](#running-on-production)
  - [Running React and NodeJS on the same server](#running-react-and-nodejs-on-the-same-server)
  ...
  - [Separating client and server](#separating-client-and-server)
    ...
    - [React as standalone](#react-as-standalone)


# Remarks

> This is the new starter template, which is based on the deprecated ["nodejs-react-starter"](https://github.com/shy2net/nodejs-react-starter) which is built on [TS.ed](https://tsed.io/). The reason the old library was deprecated is because NestJS is more documented and popular library, which has very much the same approach as React.

> This template is based on the exact same code for React, and most of the original template node code was reused and refactored to suit NestJS approach.
...
This starter template\boilerplate comes with NodeJS (typescript) and React 10. It shares models between React
and NodeJS. Both of the NodeJS and and React 10 can run on the same webserver as the NodeJS exposes all of the
default routes to React and all of the known routes to the api.

Technologies used in this template:

- React 10 (with SSR) - including unit tests (based on Jasmine + Karma)
...
To work with this template **locally (debug mode)**, follow these commands:
    ...
    npm i -g @react/cli # Install react globally (some scripts depend on ng to be globally installed)
    npm run install:all # Install all dependencies required for both NodeJS and React
...
In order to compile and build this template for your **production server** run the following:
    ...
    npm run install:all # Install all dependencies required for both NodeJS and React
    npm run build # Run the build.sh script to compile and NodeJS and React for production
    npm start
...
React 10 comes with the following features:
...
The code of React 10 is stored under the `react-src` directory.

### React services & providers
...
- `SocialLoginService` - This service is responsible for the whole social authentication (Google and Facebook), it uses `reactx-social-login` module to do so. This service can be found under `social-login` module which initializes all of the providers (which are the 3rd party social sites).
...
### React Universal (Server-Side-Rendering)

By default this template comes ready with React Universal which allows search engines to crawl your website better.
It does this on the NodeJS side after running the `npm run build` command which bundles the react code and create an react universal express ready file called `out/src/dist/server.js` which our NodeJS simply imports and initializes in the `src/app.ts` file.

in the `src/misc/react-mounter.ts` you have these two methods:
...
export function mountReact(expressApp: express.Application): void {
  ...
  // Point static path to React 2 distribution
  expressApp.use(express.static(DIST_FOLDER));

  // Deliver the React 2 distribution
...
By default the mountReact or mountReactSSR is called in the init method of `src/main.ts`, in order
to disable or enable SSR, change the related config called `REACT > USE_SSR` into true to enable SSR.
...
- React routes support (redirect to index.html of compiled React code), this means you can run you React app and API on the same container!
...
- Mount React (if required) to deliver the web interface, we will read about it later
...
read about them before diving into this README. To sum it up, if you have experience with React,
...
You can use the `shared` directory in order for NodeJS and React to share the same code to be used on both sides
...
As this template shares code between React and NodeJS, validations will happen across both platforms.
...
- React - checks that all fields are valid using the `FormValidatorDirective`, each field on your form will automatically show if it is valid or invalid
  ...
  `react-src/src/app/components/register/register.component.html`:
...
## Running React and NodeJS on the same server

This template comes with React and NodeJS bundled together and can
...
How does it work? Well it simply compiles each one seperatly and then copies the react-src output dist directory
...
// Deliever the React 2 distribution
...
NodeJS server side and React into one. This means when you run the server after the build you will have both on the same
node container.
...
script. The build will compile and copy all of the required configurations for the specified environment, and will generate the React
...
In order to run react as a standalone, simply compile it:
```bash
npm run build:react
```

The output will be projected into the `react-src/dist` directory.
